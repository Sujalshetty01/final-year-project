...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Typography, TextField, Chip, Stack } from '@mui/material';
...
const HistoryTable = ({ history, onSelect }) => {
  ...
  const filtered = history && history.length > 0
    ? history.filter(item =>
        ...
        <TableBody>
          {filtered.length > 0 ? filtered.map((item, idx) => (
            <TableRow key={idx} hover sx={{ cursor: 'pointer' }} onClick={() => onSelect(item)}>
              ...
              <TableCell>
                <Chip label={String(item.label ?? '')} color={getStatusColor(item.label)} size="small" />
              </TableCell>
