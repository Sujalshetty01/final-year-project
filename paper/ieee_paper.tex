...
For node embeddings H^{(l)} \\in R^{N \\times D_l} at layer l, the standard GCN propagation rule is written as
...
\begin{itemize}
	\item `data/` - synthetic data generation and dataset builders.
	\item `models/` - PyTorch model definitions, training scripts, and checkpoints.
	\item `backend/` - FastAPI server, inference helpers, and tests.
	\item `scripts/` - automation helpers such as `auto_local_runner.py` and conversion utilities.
\end{itemize}
